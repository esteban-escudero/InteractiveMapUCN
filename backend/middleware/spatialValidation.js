...
const spatialValidation = {
  ...
  validateBuildingLocation: (req, res, next) => {
    try {
      ...
      if (!isValid) {
        console.warn(`⚠️ Coordenadas fuera del campus: ${latitude}, ${longitude}`);
  ...
  validateRouteGeometry: (req, res, next) => {
    try {
      ...
      if (!req.body.distancia) {
        const realDistance = Math.round(TurfUtils.calculateRouteLength(coordinates));
  ...
  validateRoutePoints: (req, res, next) => {
    try {
      ...
      for (const punto of puntos_ruta) {
        ...
        if (!isValid) {
          console.warn(`⚠️ Punto de ruta fuera del campus: ${lat}, ${lng}`);
        }
      }
  ...
  optimizeRoute: (req, res, next) => {
    try {
      ...
      if (geometria.coordinates.length > 50) {
        ...
        console.log(`✅ Ruta simplificada: ${geometria.coordinates.length} → ${simplified.length} puntos`);
      }
