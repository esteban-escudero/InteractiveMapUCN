...
const routeNodesController = {
  ...
  async getRouteIntersections(req, res) {
    try {
      ...
      console.log(
        `Buscando intersecciones entre rutas ${route1Id} y ${route2Id}`
      );
      ...
      const intersections = await RouteNodes.findRouteIntersections(
        route1Id,
        route2Id
      );
  ...
  async suggestConnections(req, res) {
    try {
      ...
      const suggestions = await RouteNodes.suggestRouteConnections(
        routeId,
        maxDistance
      );
  ...
  async findNearbyNodes(req, res) {
    try {
      ...
      const nodes = await RouteNodes.findNearbyNodes(
