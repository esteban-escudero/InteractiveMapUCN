...
const RouteNetwork = ({ mapInstance, onNodeClick, onRouteClick }) => {
  ...
  const calculateSharedNodesFromFrontend = async () => {
    try {
      ...
      allPoints.forEach((point) => {
        const key = `${point.coordenadas.lng.toFixed(
          6
        )},${point.coordenadas.lat.toFixed(6)}`;
    ...
    if (Object.keys(routeConnections).length > 0) {
      ...
      const routeName =
        sharedNodes
          ...
          .find((route) => route.routeId == mostConnected[0])?.routeName ||
        "Desconocida";
  ...
  return (
    <div className="route-network-panel">
      ...
      {loading && (
        <div className="loading-message">Cargando nodos compartidos...</div>
      )}
          ...
          <div className="stats-grid">
            <div className="stat-item">
              ...
              <span className="stat-value">
                {networkAnalysis.total_nodos_compartidos}
              </span>
            </div>
            <div className="stat-item">
              <span className="stat-label">Conexiones totales:</span>
              <span className="stat-value">
                {networkAnalysis.conexiones_totales}
              </span>
            </div>
            {networkAnalysis.ruta_mas_conectada && (
              <div className="stat-item">
                ...
                <span className="stat-value">
                  {networkAnalysis.ruta_mas_conectada.nombre} (
                  {networkAnalysis.ruta_mas_conectada.conexiones})
                </span>
              </div>
            )}
          </div>
            ...
            <div
              ...
              <div className="node-info">
                <div className="node-coords">
                  {node.coordenadas.lat.toFixed(4)},{" "}
                  {node.coordenadas.lng.toFixed(4)}
                </div>
      ...
      {selectedNode && (
        <div className="node-details">
          ...
          <div className="detail-item">
            ...
            {selectedNode.coordenadas.lat.toFixed(6)},{" "}
            {selectedNode.coordenadas.lng.toFixed(6)}
          </div>
          <div className="detail-item">
