...
export const useBuildings = () => {
  ...
  const loadBuildings = useCallback(async () => {
    ...
    try {
      ...
      const buildingsArray = Array.isArray(response.data) ? response.data : 
        ...
        try {
          ...
          if (building.ubicacion) {
            if (building.ubicacion.type === "Point") {
              ...
              turfData.ubicacion_valida = SpatialUtils.isPointInPolygon(lat, lng, [
                [-71.355622, -29.967316],
                [-71.346738, -29.967316],
                [-71.346738, -29.963208],
                [-71.355622, -29.963208],
              ...
              const centroCampus = { lat: -29.965262, lng: -71.351180 };
              ...
              try {
                ...
                console.error(`❌ Error calculando área para ${building.nombre}:`, areaError);
              }
              ...
              try {
                ...
                console.error(`❌ Error calculando centroide para ${building.nombre}:`, centroidError);
              }
            }
          }
          ...
          console.error(`❌ Error en análisis Turf para edificio ${building.nombre}:`, turfError);
      ...
      console.log(`✅ ${buildingsWithTurfAnalysis.length} edificios cargados con análisis Turf`);
  ...
  const calculateBuildingAnalytics = useCallback((buildingsData) => {
    try {
      ...
      buildingsData.forEach(building => {
        ...
        analytics.edificios_por_tipo[tipo] = (analytics.edificios_por_tipo[tipo] || 0) + 1;
      ...
      // Cálculos promedios
      analytics.distancia_promedio_centro = edificiosConDistancia > 0 ? 
        Math.round(totalDistancia / edificiosConDistancia) : 0;
  ...
  const createBuilding = useCallback(async (buildingData) => {
    ...
    try {
      ...
      if (buildingData.lat && buildingData.lng) {
        ...
        if (!isValid) {
          console.warn('⚠️ Edificio creado fuera de los límites del campus');
        }
      }
  ...
  // ✅ ACTUALIZAR EDIFICIO
  const updateBuilding = useCallback(async (buildingId, buildingData) => {
    setLoading(true);
    setError(null);
    
    try {
      console.log(`✏️ Actualizando edificio ${buildingId}...`);

      const response = await buildingService.updateBuilding(buildingId, buildingData);
      
      // Actualizar estado local
      setBuildings(prev => prev.map(building => 
        building.id === buildingId ? { ...building, ...buildingData } : building
      ));
  ...
  const deleteBuilding = useCallback(async (buildingId) => {
    ...
    try {
      console.log(`🗑️ Eliminando edificio ${buildingId}...`);
  ...
  const syncWithGeoServer = useCallback(async (geoServerData) => {
    setLoading(true);
    setError(null);
  ...
  const findBuildingsNearPoint = useCallback((point, maxDistance = 200) => {
    try {
      ...
      const edificiosCercanos = buildings.filter(building => {
        try {
          if (!building.ubicacion || building.ubicacion.type !== "Point") return false;
          ...
          console.error(`❌ Error calculando distancia a edificio ${building.nombre}:`, error);
          return false;
        }
      });
      ...
      edificiosCercanos.sort((a, b) => {
        ...
        const distA = SpatialUtils.calculateDistance(point, { lat: latA, lng: lngA });
        const distB = SpatialUtils.calculateDistance(point, { lat: latB, lng: lngB });
        
        return distA - distB;
      });

      console.log(`📍 Encontrados ${edificiosCercanos.length} edificios dentro de ${maxDistance}m`);
  ...
  const getDensityStats = useCallback(() => {
    ...
    stats.densidad_edificios = (stats.total_edificios / areaCampusAprox * 10000).toFixed(2);
    stats.densidad_salas = (stats.total_salas / areaCampusAprox * 10000).toFixed(2);
  ...
  return {
    ...
    isValidBuilding: (building) => building && building.ubicacion_valida !== false,
