...
export const useGeoServer = () => {
  ...
  const loadWFSData = useCallback(async (mapInstance, layerName = 'edificio') => {
    setStatus('loading');
    setError(null);
    ...
    try {
            ...
            if (feature.geometry.type === "Polygon") {
              ...
              try {
                turfData.area_calculada = Math.round(
                  SpatialUtils.calculatePolygonArea(feature.geometry.coordinates[0])
                );
              } catch (areaError) {
      ...
      setStatus('success');
      console.log(`✅ ${featuresWithTurfAnalysis.length} features WFS cargados con Turf`);
          ...
          if (feature.geometry.type === "Polygon") {
            ...
            layer = window.L.marker([lat, lng], {
              icon: window.L.divIcon({
                html: `<div style="background-color: ${isValid ? '#27ae60' : '#e74c3c'}; 
  ...
  const createFeaturePopup = useCallback((feature) => {
    ...
    const statusText = turfInfo.ubicacion_valida ? 'Dentro del campus' : 'FUERA del campus';
  ...
  const clearMapLayers = useCallback((mapInstance) => {
    ...
    try {
      ...
      console.log('🗑️ Capas GeoServer limpiadas del mapa');
    } catch (error) {
  ...
  const getFeaturesByGeometryType = useCallback((geometryType) => {
    return features.filter(feature => 
      feature.turf?.tipo_geometria === geometryType
    );
  }, [features]);

  // ✅ OBTENER FEATURES VÁLIDOS/INVÁLIDOS
  const getFeaturesByValidity = useCallback((isValid) => {
    return features.filter(feature => 
      feature.turf?.ubicacion_valida === isValid
    );
  }, [features]);
