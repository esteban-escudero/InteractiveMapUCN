...
export const useRoutes = () => {
  ...
  const loadRoutes = useCallback(async () => {
    ...
    try {
      ...
      const routesArray = Array.isArray(response) ? response : 
        ...
        try {
          ...
          if (route.geometria && route.geometria.coordinates) {
            ...
            if (coordinates.length >= 2) {
              ...
              turfData.tiempo_estimado_turf = Math.round(turfData.distancia_turf / 80);
          ...
          if (route.puntos_ruta && Array.isArray(route.puntos_ruta)) {
            ...
            const puntosInicio = route.puntos_ruta.filter(p => p.tipo_punto === 'inicio');
            const puntosFin = route.puntos_ruta.filter(p => p.tipo_punto === 'fin');
            const puntosIntermedios = route.puntos_ruta.filter(p => p.tipo_punto === 'intermedio');
          ...
          console.error(`❌ Error en análisis Turf para ruta ${route.nombre}:`, turfError);
      ...
      console.log(`✅ ${routesWithTurfAnalysis.length} rutas cargadas con análisis Turf`);
  ...
  const calculateRouteAnalytics = useCallback((routesData) => {
    try {
      ...
      routesData.forEach(route => {
        ...
        analytics.rutas_por_tipo[tipo] = (analytics.rutas_por_tipo[tipo] || 0) + 1;
      ...
      // Cálculos promedios
      analytics.distancia_promedio = Math.round(analytics.total_distancia / analytics.total_rutas);
      analytics.puntos_promedio = (analytics.total_puntos / analytics.total_rutas).toFixed(1);
  ...
  const updateRoute = useCallback(async (routeId, routeData) => {
    ...
    try {
      console.log(`✏️ Actualizando ruta ${routeId} con Turf...`);
  ...
  const calculateOptimalRoute = useCallback(async (origen, destino, tipo_ruta = 'peatonal') => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (distanciaDirecta < 10) { // 10 metros
        throw new Error('Los puntos de origen y destino están demasiado cerca');
      }

      // Usar el servicio del backend para cálculo de ruta
      const response = await routeService.calculateRoute(origen, destino, tipo_ruta);
      ...
      console.error('❌ Error en useRoutes.calculateOptimalRoute:', errorMessage);
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
  ...
  const findRoutesNearPoint = useCallback((point, maxDistance = 100) => {
    try {
      ...
      const rutasCercanas = routes.filter(route => {
        try {
          ...
          console.error(`❌ Error calculando distancia a ruta ${route.nombre}:`, error);
          return false;
        }
      });

      console.log(`📍 Encontradas ${rutasCercanas.length} rutas dentro de ${maxDistance}m`);
  ...
  const getPerformanceStats = useCallback(() => {
    ...
    stats.eficiencia_geometrica = rutasConEficiencia > 0 ? 
      (totalEficiencia / rutasConEficiencia).toFixed(1) : 0;
